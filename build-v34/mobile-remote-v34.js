(() => {
  ...
  async function api(path, body){
    ...
    try{
      const r=await fetch(API+path,{method:'POST',cache:'no-store',signal:ctl?ctl.signal:undefined,headers:{'Content-Type':'application/json'},body:JSON.stringify({...a,...body,deviceType:'MOBILE',clientType:'MOBILE',mobileVersion:'1.0.34'})});
  ...
  async function disconnectRemote(){
    try{ await api('/remote/mobile-disconnect',{}); }catch(_){}
  }
  ...
  function closeRemote(){
    if(opened) void disconnectRemote();
    opened=false;
